...
export type Api = {
  ...
  updateIncomeStream: (id: string, patch: { source?: string; kind?: string }) => Promise<unknown>;
  updateOutflowStream: (id: string, patch: { name?: string; category?: string; kind?: string }) => Promise<unknown>;
...
const liveApi: Api = {
  ...
  updateIncomeStream: (id, patch) => req("PATCH", `/income/${id}`, patch),
  updateOutflowStream: (id, patch) => req("PATCH", `/outflows/${id}`, patch),
...
const demoApi: Api = {
  ...
  updateIncomeStream: async () => ({ ok: true }),
  updateOutflowStream: async () => ({ ok: true }),
